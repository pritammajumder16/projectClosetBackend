...
userRoleUpdate.get(
  ...
  async (req, res, next) => {
    ...
    const result = await userModel.findOneAndUpdate({email:user.email},{$set:obj},{upsert:false,new:true});
...
userRoleUpdate.get("/allUsers", async (req, res, next) => {
    ...
    const result = await userModel.find({}).sort({userName:1}).skip(startIndex).limit(pageSize);
