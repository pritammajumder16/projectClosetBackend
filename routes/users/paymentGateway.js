...
const generateOrderId = ()=>{const timestamp = new Date().getTime().toString(); 
...
//   console.log(orderId)
  ...
  axios
    ...
    //   console.log("response", response.data.data.instrumentResponse.redirectInfo.url,req.body.orderId);
      return sendDataSuccess(res,response.data.data.instrumentResponse.redirectInfo.url)
    })
...
paymentRouter.post("/redirectUrl", async function (req, res) {
  ...
  if (
    ...
    const user = await userModel.findOne({"lastOrder.orderId":req.body.transactionId})
    if(!user){
    ...
    if (req.body.transactionId) {
      ...
      axios
        ...
        .then(async function (response) {
            ...
            userModel.updateOne({ email: user.email }, { $set: { "cart": [] } })
            .then(result => {
