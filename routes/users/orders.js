...
orderRouter.get("/getOrders", async (req, res, next) => {
  if (req.query.email) {
    ...
    const startIndex = parseInt(req.query.pageIndex-1)*parseInt(req.query.pageSize)
    const finalResult = await orderModel.find({status:{$ne:"cancelled"} }).skip(startIndex).limit(parseInt(req.query.pageSize));
    const count = await orderModel.find({status:{$ne:"cancelled"} }).count()
...
orderRouter.post("/orderStatus", async (req, res, next) => {
    if (req.body.orderId && req.body.status && ["cancelled","pending", "shipped","completed"].includes(req.body.status)) {
        console.log({ orderId: req.body.orderId })
      const finalResult = await orderModel.findOneAndUpdate({ orderId: req.body.orderId },{$set:{status:req.body.status}});
      return sendDataSuccess(res, finalResult);
    } 
